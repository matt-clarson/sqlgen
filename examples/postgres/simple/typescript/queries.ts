...
export type GetTable4Result = {
    id: string;
    created: Date;
    updated: Date | null;
};

export async function getTable4(
    dispatch: Dispatcher,
): Promise<GetTable4Result[]> {
    const query = `
        SELECT * FROM table_4;
    `;

    const result = await dispatch(query);
    return mapRows<GetTable4Result>(result.keys, result.rows);
}

export type GetTable5Result = {
    id: number;
    words: string[] | null;
};

export async function getTable5(
    dispatch: Dispatcher,
): Promise<GetTable5Result[]> {
    const query = `
        SELECT * FROM table_5;
    `;

    const result = await dispatch(query);
    return mapRows<GetTable5Result>(result.keys, result.rows);
}
...
export type InsertTable4Arg = {
    id: string;
    created: Date;
    updated: Date | null;
};

export async function insertTable4(
    dispatch: Dispatcher,
    arg: InsertTable4Arg,
): Promise<void> {
    const query = `
...
export type InsertTable5Arg = {
    id: number;
    words: string[] | null;
};

export async function insertTable5(
    dispatch: Dispatcher,
    arg: InsertTable5Arg,
): Promise<void> {
    const query = `
